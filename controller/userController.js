...
module.exports = {
    register: async (req, res, next) => {
        try {
            if (req.body.email.includes('@') && req.body.password.length >= 6 && req.body.password.match(/[a-z]/ig) && req.body.password.match(/[0-9]/ig)){
                let insertSQL = `Insert into users (username, nama, email, password, telp) 
                values (${db.escape(req.body.username)}, ${db.escape(req.body.nama)}, ${db.escape(req.body.email)}, ${db.escape(req.body.password)}, ${db.escape(req.body.telp)});`
                let regis = await dbQuery(insertSQL)
                let getUser = await dbQuery(`Select * from users where idusers = ${regis.insertId}`)
                let { idusers, username, nama, email, password, telp, idrole, idstatus_user } = getUser[0]

                //TOKEN
                let token = createToken({ idusers, username, nama, email, password, telp, idrole, idstatus_user })

                res.status(201).send({ idusers, username, nama, email, password, telp, idrole, idstatus_user, token })
            } else {
    ...
    login: async (req, res, next) => {
        try {
            if (req.body.email && req.body.password){
                ...
                res.status(200).send({ idusers, username, nama, email, password, telp, idrole, idstatus_user, token })
            }
        } catch (error) {
            next(error)
        }
    ...
    addKurir: async (req, res, next) => {
        try {
            if (req.body.email.includes('@') && req.body.password.length >= 6 && req.body.password.match(/[a-z]/ig) && req.body.password.match(/[0-9]/ig)) {
                let id_role = 2
                let insertSQL = `Insert into users (username, nama, email, password, telp, idrole) 
                values (${db.escape(req.body.username)}, ${db.escape(req.body.nama)}, ${db.escape(req.body.email)}, ${db.escape(req.body.password)}, ${db.escape(req.body.telp)}, ${id_role});`
